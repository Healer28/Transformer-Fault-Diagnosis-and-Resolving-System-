...
const int currentPin = 27;  // Pin connected to ACS712 output
const float sensitivity = 0.185;  // Sensitivity of ACS712 5A (185 mV/A)
const float offsetVoltage = 2.360;  // Adjust based on actual measured offset
...
// #define CURRENT_SENSOR_PIN 27  // Analog pin connected to OUT of ACS712
// const float ACS712_SENSITIVITY = 185.0;  // mV per Amp for 5A version
// const float V_REF = 3.3 ;                 // ESP32 reference voltage
// const int ADC_RESOLUTION = 4095;         // ADC resolution for ESP32
// const float ZERO_CURRENT_OFFSET = 2.5;   // Zero current voltage in volts
...
//   float voltage = (sensorValue / float(ADC_RESOLUTION)) * V_REF;  // Map ADC value to voltage
//   float current = (voltage - ZERO_CURRENT_OFFSET) / (ACS712_SENSITIVITY / 1000.0);  // Current in Amps
